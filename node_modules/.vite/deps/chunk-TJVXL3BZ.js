...
async function x(l2) {
  ...
  if ("heatmap" === l2.type) {
    ...
    for (const e of l2.attributes) await w(t, e.field || e.valueExpression, [e.color]);
  ...
  } else if ("unique-value" === l2.type) if ("predominance" === ((_b = l2.authoringInfo) == null ? void 0 : _b.type)) for (const e of l2.uniqueValueInfos ?? []) await w(t, e.value.toString(), [h(e.symbol, null)]);
  else {
    const e = (l2.uniqueValueInfos ?? []).map((e2) => h(e2.symbol, null)), { field: i2, field2: o, field3: a2, valueExpression: n2 } = l2;
    (i2 || n2) && await w(t, i2 || n2, e), o && await w(t, o, e), a2 && await w(t, a2, e);
  }
  else if ("class-breaks" === l2.type) {
    const e = l2.classBreakInfos.map((e2) => h(e2.symbol, null)), { field: i2, valueExpression: o } = l2;
    await w(t, i2 ?? o, e);
  } else "simple" === l2.type && await w(t, "default", [h(l2.symbol, null)]);
  return "defaultSymbol" in l2 && l2.defaultSymbol && await w(t, "default", [h(l2.defaultSymbol, null)]), t.forEach((l3, i2) => {
    const o = i(l3.filter(Boolean), (e, l4) => JSON.stringify(e) === JSON.stringify(l4));
    t.set(i2, o);
  }), t;
}
...
//# sourceMappingURL=chunk-TJVXL3BZ.js.map
